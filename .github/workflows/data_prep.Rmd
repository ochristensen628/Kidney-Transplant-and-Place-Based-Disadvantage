...
single_registrations <- df_cand_kipa %>%
  ...
  mutate(min_list_date = min(CAN_LISTING_DT),  
         ...
         outcome = case_when(
           DON_TY == "C" ~ "DDKT",
           DON_TY == "L" ~ "LDKT",
           CAN_REM_CD %in% c(13, 8) ~ "removed/died",
           TRUE ~ "censored"
         ))
```
...
} ##still not totally clear on why we do this
...
for(i in 1:(nrow(multiple_registrations)-1)) { ##loop from second to last row
...
sequential_lists <- multiple_registrations %>%
  ...
  mutate(min_list_date = CAN_LISTING_DT,
         ...
         outcome = case_when(
           DON_TY == "C" ~ "DDKT",
           DON_TY == "L" ~ "LDKT",
           CAN_REM_CD %in% c(13, 8) ~ "removed/died", ##again, should this be all CAN_REM_CD? or just 8 and 13 (since we included the rest in censored) - changed
           TRUE ~ "censored"
  ...
  multiple_registrations %>%
    ...
    mutate(wait_time = case_when(
      ...
      outcome = case_when(
        DON_TY == "C" ~ "DDKT",
        DON_TY == "L" ~ "LDKT",
        CAN_REM_CD %in% c(13, 8) ~ "removed/died", ##same question - CAN_REM_CD should be all? or just 8, 13? - changed
...
df_cand_kipa <- df_cand_kipa %>%
  select(PX_ID, PERS_ID, living_donor, raw_epts, percentile_epts, top_percentile_epts, 
         ...
         CAN_DEATH_DT, PERS_SSA_DEATH_DT, PERS_OPTN_DEATH_DT, CAN_RACE_SRTR, CAN_AGE_AT_LISTING, CAN_ETHNICITY_SRTR, CAN_TIEBREAKER_DT, CAN_GENDER, CAN_ABO, CAN_MAX_MM_ABDR) ##only leaves in necessary variables
```
...
df_cand_kipa_w_scores <- df_cand_kipa_w_scores %>%
  mutate(status = case_when(
    ...
    outcome == "removed/died" & CAN_REM_CD %in% c(8, 13) & survival_time < 1826 ~ 2, # Removed/died due to deterioration
    TRUE ~ 0))
...
df_cand_kipa_w_scores <- df_cand_kipa_w_scores %>%
  mutate(transplant_outcome = case_when(
    ...
    status == "censor" ~ 0,
    status == "transplant" & survival_time < 1826 ~ 1,
    .default = 0),
    death_outcome = case_when(
      survival_time == 1826 ~ 0,
      status == "censor" ~ 0,
      status == "death/deteriorate"  & survival_time < 1826 ~ 1,
      .default = 0
    ))
```
...
```{r}
write.csv(df_cand_kipa_w_scores, "df_cand_kipa_w_scores.csv", row.names = FALSE)

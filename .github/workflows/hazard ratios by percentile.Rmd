...
# Fit the Cox model with restricted cubic splines
model <- cph(Surv(survival_time, death_outcome) ~ rcs(ADI_percentile_num, 4), data = df_cand_kipa_w_scores, x = TRUE, y = TRUE)
...
```{r}
min(df_cand_kipa_w_scores$ADI_percentile_num, na.rm = TRUE)
```
...
# Fit the Cox model with restricted cubic splines
model <- cph(Surv(survival_time, death_outcome) ~ rcs(SVM_percentile_num, 4), data = df_cand_kipa_w_scores, x = TRUE, y = TRUE)

# Generate new data for prediction
new_data <- data.frame(SVM_percentile_num = seq(1,100))
...
# Fit the Cox model with restricted cubic splines
model <- cph(Surv(survival_time, transplant_outcome) ~ rcs(SVM_percentile_num, 4) + dial_time, data = df_cand_kipa_w_scores, x = TRUE, y = TRUE)

# Generate new data for prediction
new_data <- data.frame(SVM_percentile_num = seq(1,100), dial_time = 0)

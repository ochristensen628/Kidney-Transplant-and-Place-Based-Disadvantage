...
SVI <- distinct(SVI, ZIP, .keep_all = T) %>% select(RPL_THEMES, ZIP)
...
```{r}
df_cand_kipa_w_scores <- left_join(df_cand_kipa, select(cand_zip, SVM_SCORE, SVM_PERCENTILE, PX_ID, SVI_percentile,  dci_percentile),
                              by = c("PX_ID" = "PX_ID"))
...
non_missing_summary <- missing_summary(df_cand_kipa_w_scores %>% filter(!is.na(SVI_decile) & !is.na(SVM_decile) & !is.na(DCI_decile)), "No Missing")


# Combining all tables into one
combined_summary <- tbl_merge(
  list(non_missing_summary, svi_summary,  svm_summary, dci_summary),
  tab_spanner = c("**No Missing**", "**SVI**",  "**SVM**", "**DCI**")
)

# Display the combined summary
combined_summary

df_cand_kipa_w_scores %>% filter(!is.na(SVI_decile) & !is.na(SVM_decile) & !is.na(DCI_decile))
```
...
df_cand_kipa_w_scores %>% select(SVM_PERCENTILE, SVI_percentile, dci_percentile)
```
